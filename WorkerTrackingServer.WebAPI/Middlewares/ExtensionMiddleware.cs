...
public static class ExtensionsMiddleware
{
    ...
    public static void CreateMasterAdmin(WebApplication app)
    {
        using (var scoped = app.Services.CreateScope())
        {
            var userManager = scoped.ServiceProvider.GetRequiredService<UserManager<AppUser>>();
            if (!userManager.Users.Any(p => p.Email == "ihsanerendelibas@gmail.com"))
            {
                AppUser user = new()
                {
                    ...
                    Role = UserRoleSmartEnum.MasterAdmin,
                    EmailConfirmed = true,
                    IsDeleted = false,
                    CreatedBy = "System",
                    CreatedDate = DateTime.Now,
